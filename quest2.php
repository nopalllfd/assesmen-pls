...
<?php
    ob_start();
    $questions = [
        "2.	Mana profesi yang menurut anda paling cocok untuk anda ?" => ["a. Seniman", "b. Atlet", "c. Dosen", "d. Politikus"],
...
<html>
<head>
    <title>Asesmen Peserta PLS</title>
        <link
      rel="stylesheet"
      href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.2/dist/css/bootstrap.min.css"
      integrity="sha384-xOolHFLEh07PJGoPkLv1IbcEPTNtaed2xpHsD9ESMhqIYd0nLMwNLD69Npy4HI+N"
        ...
        foreach ($correct_answers as $key => $correct_answer) {
            ...
            if (isset($answers[$question])) {
                ...
                switch ($answers[$question]) {
                    case 'a':
                        $_SESSION['countA']++;
                        $sql = "UPDATE `question` SET `a2` = '$answers[$question] ' WHERE `question`.`id_user` = '$noreg';";
                        $db->query($sql);
                        break;
                    case 'b':
                        $_SESSION['countB']++;
            ...
            header("Location: quest3.php?message=$_GET[message]");
