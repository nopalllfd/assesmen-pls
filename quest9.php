...
ob_start();
$questions = [
    "9.	Ketika menonton Konten Video di YouTube, kira-kira yang mana yang paling anda nikmati untuk ditonton ?
" => ["a. Konten Video Dance", "b. Konten Video Street Basketball", "c. Konten Video Fakta Unik", "d. Konten Video Konspirasi"],
    ...
    <form class="col-10 mx-auto" method="POST" action="">
        ...
        foreach ($questions as $question => $choices) {
            ...
            foreach ($choices as $choice) {
                ...
                echo "<label><input style='transform: scale(4)' class='radio p-5' type='radio' name='answers[$question]' value='$value' required> <span class='p-5 mx-auto'>$choice</span></label><br>";
                echo "<br>";
            }
        ...
        foreach ($correct_answers as $key => $correct_answer) {
            ...
            if (isset($answers[$question])) {
                ...
                switch ($answers[$question]) {
                    case 'a':
                        $_SESSION['countA']++;
                        $sql = "UPDATE `question` SET `a9` = '$answers[$question] ' WHERE `question`.`id_user` = '$noreg';";
            ...
            header("Location: final.php?message=$_GET[message]");
